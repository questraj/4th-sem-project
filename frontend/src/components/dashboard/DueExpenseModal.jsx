...
import DueExpenseModal from '@/components/dashboard/DueExpenseModal'; // NEW IMPORT
...
const Dashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // --- NEW: Fetch Due Expenses ---
      try {
        const dueRes = await api.get('/expense/getDueExpenses.php');
        if (dueRes.data.success) {
          setDueExpenses(dueRes.data.data);
        }
      } catch (e) {
        console.warn("Due expenses fetch failed", e);
      }
  ...
  // Handlers for Due Expense Flow
  const handleDueSuccess = () => {
    // Remove the currently processed expense from the list locally
    setDueExpenses(prev => prev.filter((_, idx) => idx !== currentDueIndex));
    // Refresh stats to show the newly added expense immediately
    fetchData(); 
  };

  const handleDueSkip = () => {
    // Move to next item without processing
  ...
  return (
    <DashboardLayout>
      ...
      {/* 4. Due Expense Modal (Auto-popup for future expenses) */}
